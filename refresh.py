#!/usr/bin/env python3
"""
SN25 (Mainframe) Token Intelligence — data refresher.
...
Usage:
    python3 refresh.py
    TAOSTATS_API_KEY=... TAO_KEYS=... python3 refresh.py   # env overrides
"""
...
NETUID   = 25
TARGETS  = (93, 11, 30)   # Bitcast, TrajectoryRL, Endure Network — pinned comps
NEIGHBORS = 3             # ranks above/below SN25 to show
...
def ts_get(path, **params):
    ...
    for attempt in range(6):
        ...
        try:
            ...
            if e.code in (403, 429):      # throttled — back off and retry
...
def ts_validators():
    """coldkey -> validator name across all network validators (pages)."""
    out = {}
    page = 1
    while page <= 6:
        d = ts_get("/api/dtao/validator/latest/v1", limit=200, page=page)
...
def ts_identity_map():
    """coldkey -> (name, is_validator) from ALL on-chain identities + exchanges (few paged calls)."""
    out = {}
    ...
    page = 1
    while page <= 12:
        d = ts_get("/api/identity/latest/v1", limit=200, page=page)
        if not d or not d.get("data"):
            break
        for it in d["data"]:
            ck = (it.get("address") or {}).get("ss58")
            nm = it.get("name")
            if ck and nm and ck not in out:
                out[ck] = (nm, bool(it.get("validator_hotkey")))
        if not (d.get("pagination") or {}).get("next_page"):
            break
        page += 1
    return out
...
def build_leaderboard(token):
    ...
    rows = [{"rank": p["_rank"], "netuid": p["netuid"], "name": p["name"],
             ...
             "is_sn25": p["netuid"] == NETUID, "is_target": p["netuid"] in TARGETS}
            for p in sorted(picks.values(), key=lambda x: x["_rank"])]
    return {"sn25_rank": r, "total": len(ranked), "rows": rows}
...
def main():
    ...
    for i, h in enumerate(hold, 1):
        ...
        if ck in val_map:                        # network/SN25 validator
            is_val = True
            if label is None:
    ...
    data = {
        ...
        "subnet": {"netuid": NETUID, "name": sub["name"], "price_usd": price_usd, "price_tao": price_tao,
                   ...
                   "holder_count": holder_total, "rank": leaderboard["sn25_rank"]},
    ...
    print("data.json @ %s | SN%d %s $%.4f | rank #%d | %d holders (%d tagged, %d val) | %s | id_map=%d" % (
        data["generated_at"], NETUID, data["subnet"]["name"], price_usd, leaderboard["sn25_rank"],
